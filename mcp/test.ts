...
const good = `<html><head><link rel="stylesheet" href="${css.url}"></head><body class="bg-background text-foreground antialiased"><div data-pick="default"></div><section data-state="default"><div class="flex items-center gap-2 bg-primary/5 text-sm">${comp.snippets[0]?.html ?? ""}</section><section data-state="empty" hidden></section><section data-state="loading" hidden></section><section data-state="error" hidden></section></body></html>`;
